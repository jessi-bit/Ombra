...
let TestSum () =
    // (+ 1 41)
    let exp = List[Symbol "+";  Float 1.0 ; Float 2.0]
    let env = (Env operations)
    printfn "Env %A" env
    match (eval exp env) with
        | Float f -> Assert.AreEqual (3.0, f)
        | _ -> Assert.Fail()
    Assert.Pass()
...
// let TestSub () =
//     // (- 43 1)
//     let exp = List[Symbol "-"; Float 43.0; Float 1.0]
//     let env = Env operations
//     match (eval exp env) with
//         | Float f -> Assert.AreEqual (42.0, f)
//         | _ -> Assert.Fail()
//     Assert.Pass()
...
// let TestSumWithVars () =
//     // (+ x y 5)
//     let exp = List [Symbol "+"; Symbol "x"; Symbol "y"; Float 5.0]
//     let env = Env (Map.add "x" (Float 2.0) (Map.add "y" (Float 2.0) operations))
//     match (eval exp env) with
//         | Float f -> Assert.AreEqual (9.0, f)
//         | _ -> Assert.Fail()
//     Assert.Pass()
...
// let TestMul () =
//     // (* 5 5)
//     let exp = List [Symbol "*"; Float 5.0; Float 5.0]
//     let env = Env operations
//     match (eval exp env) with
//         | Float f -> Assert.AreEqual (25, f)
//         | _ -> Assert.Fail()
//     Assert.Pass()
